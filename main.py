...
def process_message(msg, message_id=None, chat_id=None):
    ...
    if chat_id is None: chat_id = msg.chat.id
    if message_id is None:
        tmp = bot.reply_to(msg, "⏳ يرجى الانتظار...")
            ...
            # قائمة رسائل انتظار متحركة
                ...
                if cards:
                    ...
                    bot.send_message(
                        ...
                        "يرجى استخدام البطاقات بانتظام لتعزيز استيعاب المعلومات.\n"
                        "─── ⋆⋅☆⋅⋆ ──\n"
                        "#تنويه: مراجعة البطاقات غداً تساعد في ترسيخ المعرفة بشكل أفضل.",
                        reply_to_message_id=message_id
                    )
                    with state_lock:
                        user_states.pop(uid, None)
        ...
        # ============================
            ...
            if isinstance(quizzes, list) and len(quizzes) > 0:
                try:
                    ...
                    quiz_msg = (
                    f"✨📝 إختبارك جاهز! ⏰✅\n"
                    f"<b>تم توليد الاختبار بنجاح.</b>\n"
                    f"<b>عدد الأسئلة:</b> {len(quizzes)}\n"
                    f"<b>الزمن المقدر:</b> {estimated_time // 60} دقيقة و {estimated_time % 60} ثانية\n"
