...
def save_request(msg):
    c.execute('INSERT INTO requests (user_id, username, file_id, status) VALUES (?, ?, ?, ?)', (
        msg.from_user.id,
...
def generate_Medical_quizzes(content: str, major: str, user_id: int, num_quizzes: int = 10):
    # وصف احترافي متكامل مع دعم الصور
    prompt = (
        f"أنت خبير في التقييم الطبي والتعليم الطبي. مهمتك هي إنشاء اختبار طبي متكامل لطلاب كلية الطب من نص مرجعي.\n\n"
        
        "## التعليمات الصارمة:\n"
        "1. قم بتوليد اختبار متوازن يقيس المهارات المعرفية الأساسية في الطب:\n"
        "   - 70% أسئلة المعرفة الأساسية (التشريح، الفيزيولوجيا، الكيمياء الحيوية)\n"
        "   - 30% أسئلة حل المشكلات السريرية (تشخيص حالات، تفسير نتائج)\n"
        "2. التركيز على المجالات الطبية الأساسية:\n"
        "   - الطب الباطني - الجراحة - طب الأطفال - أمراض النساء والتوليد\n"
        "   - علم الأمراض (Pathology) - علم الأدوية - علم الأحياء الدقيقة\n"
        "3. معايير الأسئلة:\n"
        "   - كل سؤال يجب أن يعتمد فقط على المعلومات الموجودة في النص المقدم\n"
        "   - الأسئلة السريرية يجب أن تقدم سيناريوهات واقعية قصيرة (2-3 جمل)\n"
        "   - تجنب الأسئلة التافهة أو ذات الإجابات الواضحة\n"
        "   - الصعوبة: متناسبة مع مستوى طالب سنة ثالثة/رابعة طب\n"
        "4. للأسئلة التي تتطلب صورًا توضيحية (مثل الهستولوجي أو التشريح):\n"
        "   - أضف حقل 'image_url' يحتوي على رابط مباشر لصورة مناسبة\n"
        "   - اختر صورًا من مصادر موثوقة (مثل ويكيميديا كومنز)\n"
        "   - تأكد من أن الصورة واضحة وذات صلة مباشرة بالسؤال\n\n"
        "## هيكل الإخراج المطلوب (JSON فقط):\n"
        "{{\n"
        "  \"title\": \"اختبار طبي في [المجال]\",\n"
        "  \"questions\": [\n"
        "    {{\n"
        "      \"id\": 1,\n"
        ...
        "      \"correct_index\": 0,\n"
        "      \"image_url\": \"رابط الصورة (اختياري)\"  # <-- أضف هذا الحقل الجديد\n"
        "    }},\n"
        "    {{\n"
        "      \"id\": 2,\n"
        "      \"type\": \"problem_solving\",\n"
        "      \"question\": \"وصف حالة سريرية\",\n"
        ...
        "## مواصفات الإخراج:\n"
        " اللغة إنجليزية\n"
        "- العدد الإجمالي: 10 أسئلة (7 اختيار متعدد، 3 حل مشكلات)\n"
        "- أسئلة حل المشكلات يجب أن تكون من النوع:\n"
        "   * تفسير نتائج مختبر\n"
        "   * تشخيص بناء على الأعراض\n"
        "   * اختيار الفحوصات التشخيصية المناسبة\n"
        "- الترتيب: تناوب بين الأنواع (ليس بالضرورة متتالية)\n\n"
        f"## النص المرجعي:\n{content}"
        "## أمثلة توضيحية للأسئلة مع الصور:\n"
        "1. سؤال هستولوجي:\n"
        "{{\n"
        "  \"id\": 3,\n"
        "  \"type\": \"multiple_choice\",\n"
        "  \"question\": \"Identify the tissue type shown in the image:\",\n"
        "  \"image_url\": \"https://upload.wikimedia.org/wikipedia/commons/thumb/3/3c/Histology_of_cardiac_muscle.jpg/640px-Histology_of_cardiac_muscle.jpg\",\n"
        ...
        "  \"id\": 4,\n"
        "  \"type\": \"problem_solving\",\n"
        "  \"question\": \"Based on the skin lesion shown in the image, what is the most likely diagnosis?\",\n"
    ...
    # استخراج ومعالجة JSON
    clean_json_str = extract_json_from_string(raw_response)
    
    if not clean_json_str:
        logging.error(f"❌ فشل استخراج JSON. الإخراج الخام:\n{raw_response}")
        return None
    ...
    try:
        ...
        if "title" not in quiz_data or "questions" not in quiz_data:
            raise ValueError("الهيكل الأساسي للـ JSON غير صحيح")
        
        # التحقق من كل سؤال
        for i, question in enumerate(quiz_data["questions"]):
            # تعيين ID إذا لم يكن موجوداً
            ...
            if question["type"] == "multiple_choice":
                if len(question.get("options", [])) != 4:
                    raise ValueError(f"سؤال {i+1}: يحتاج إلى 4 خيارات")
                if not 0 <= question.get("correct_index", -1) <= 3:
                    raise ValueError(f"سؤال {i+1}: مؤشر الإجابة غير صالح")
            
            elif question["type"] == "problem_solving":
                if "correct_answer" not in question:
                    raise ValueError(f"سؤال {i+1}: يحتاج إلى إجابة صحيحة")
                
            else:
                raise ValueError(f"سؤال {i+1}: نوع غير معروف '{question['type']}'")
            
            # التحقق من روابط الصور إذا وجدت
            if "image_url" in question:
                url = question["image_url"]
                if not url.startswith(("http://", "https://")):
                    raise ValueError(f"سؤال {i+1}: رابط الصورة غير صالح")
                if not any(domain in url for domain in ["wikimedia", "openstax", "nih.gov"]):
                    logging.warning(f"⚠️ رابط صورة غير موثوق في السؤال {i+1}: {url}")
        
        # إضافة المجال في العنوان إذا لم يكن موجوداً
        if major not in quiz_data["title"]:
            quiz_data["title"] = f"اختبار طبي في {major}"
            
        quiz_id = save_quiz_to_db(quiz_data, user_id)
        quiz_data['db_id'] = quiz_id  # إضافة ID للرجوع إليه لاحقاً
...
def store_quiz(user_id, quizzes):
    try:
        ...
        c.execute('''INSERT INTO user_quizzes 
                   (user_id, quiz_data, quiz_code, created_at) 
                   VALUES (?, ?, ?, ?)''',
                   (user_id, json.dumps(quizzes), quiz_code, datetime.now().isoformat()))
...
class QuizManager:
    ...
    def start_quiz(self, chat_id, quiz_code, bot2, message_id=None):
        ...
        if not result:
            if message_id:
                bot2.edit_message_text(chat_id=chat_id, message_id=message_id, text="❌ الاختبار غير موجود أو منتهي الصلاحية")
            else:
                bot2.send_message(chat_id, "❌ الاختبار غير موجود أو منتهي الصلاحية")
            return False
        ...
        if message_id:
            ...
            bot2.send_message(chat_id, f"تم توليد {len(formatted_quizzes)} سؤالاً، استعد للإختبار")
...
def send_quiz_to_user(chat_id, quiz_data, message_id=None):
    ...
    quiz_url = f"https://yourdomain.com/quiz/{quiz_data['db_id']}"
...
def unified_start_handler(message):
    ...
    # ✅ إذا لم يوجد باراميتر، إرسال رسالة ترحيب
    if len(args) == 1:
    ...
    # ✅ محاولة بدء الاختبار
    if not quiz_manager.start_quiz(chat_id, quiz_code, bot2):
        bot2.edit_message_text(
            chat_id=chat_id,
...
def handle_main_menu(c):
    ...
    if data == "go_generate":
        ...
        buttons = [
            ("🩺 الطب والصحة", "category_العلوم_الصحية_page1"),
            ("🛠️ الهندسة", "category_الهندسة_page1"),
            ("💻 الحاسوب", "category_علوم_الحاسوب_page1"),
            ("📊 الإدارة", "category_العلوم_الادارية_page1"),
            ("🗣️ اللغات", "category_اللغات_page1"),
        ]
        
        # إضافة الأزرار بنظام صفوف
        for i in range(0, len(buttons), 2):
            row = buttons[i:i+2]
            keyboard.row(
                InlineKeyboardButton(row[0][0], callback_data=row[0][1]),
                InlineKeyboardButton(row[1][0], callback_data=row[1][1]) if len(row) > 1 else None
            )
        
        # أزرار التنقل والإجراءات
        keyboard.row(InlineKeyboardButton("❓ تخصص آخر", callback_data="major_custom"))
        keyboard.row(InlineKeyboardButton("➡️ المزيد من التخصصات", callback_data="go_next"))
        keyboard.row(InlineKeyboardButton("🏠 القائمة الرئيسية", callback_data="go_back_home"))
        ...
        try:
            ...
            print(f"خطأ في عرض الصفحة الأولى: {e}")

    elif data == "anki":
        bot.answer_callback_query(c.id)
        
        # 👇 إرسال رسالة اختيار طريقة الإنشاء
        choice_markup = types.InlineKeyboardMarkup()
        choice_markup.row(
                types.InlineKeyboardButton("📝 توليد يدوي", callback_data=f"manual_anki"),
            types.InlineKeyboardButton("🤖 توليد آلي", callback_data=f"ai_anki")
        )
        ...
        # إنشاء لوحة مفاتيح متعددة الصفحات
        keyboard = InlineKeyboardMarkup(row_width=2)
    
        # تعريف الفئات الرئيسية
        main_categories = [
            ("🩺 العلوم الصحية", "spec_category_health"),
            ("🛠️ الهندسة", "spec_category_engineering"),
            ("💻 علوم الحاسوب", "spec_category_computer"),
            ("📊 العلوم الإدارية", "spec_category_business"),
            ("🗣️ اللغات", "spec_category_languages"),
            ("📿 العلوم الإسلامية", "spec_category_islamic"),
            ("⚖️ القانون والسياسة", "spec_category_law"),
            ("🔬 العلوم الطبيعية", "spec_category_science"),
            ("🎨 الفنون", "spec_category_arts"),
            ("👩‍🏫 التربية", "spec_category_education"),
            ("❓ تخصص آخر", "set_major_custom")
        ]
        ...
        # أزرار التنقل
        keyboard.row(InlineKeyboardButton("⬅️ رجوع", callback_data="go_account_settings"))
    ...
    elif data.startswith("spec_category_"):
        category = data.replace("spec_category_", "")

        # قاموس شامل للتخصصات الفرعية
        sub_specialties = {
            "health": [
                ("🧬 الطب البشري", "set_major_طب_بشري"),
                ("💊 الصيدلة", "set_major_صيدلة"),
                ("🏥 التمريض", "set_major_تمريض"),
                ("🔬 علوم المختبرات", "set_major_علوم_مختبرات"),
                ("🦷 طب الأسنان", "set_major_طب_أسنان"),
            ],
            "engineering": [
                ("⚙️ الميكانيكية", "set_major_هندسة_ميكانيكية"),
                ("💡 الكهربائية", "set_major_هندسة_كهربائية"),
                ("🏗️ المدنية", "set_major_هندسة_مدنية"),
                ("🔧 البرمجيات", "set_major_هندسة_برمجيات"),
                ("📡 الاتصالات", "set_major_هندسة_اتصالات"),
                ("⚗️ الكيميائية", "set_major_هندسة_كيميائية"),
                ("🛰️ الفضائية", "set_major_هندسة_فضائية"),
                ("🛠️ الصناعية", "set_major_هندسة_صناعية"),
                ("🌉 الإنشائية", "set_major_هندسة_إنشائية"),
                ("🛢️ البترول", "set_major_هندسة_بترول"),
                ("🌿 البيئية", "set_major_هندسة_بيئية"),
                ("🧭 الميكاترونكس", "set_major_هندسة_ميكاترونكس"),
                ("🔌 الإلكترونية", "set_major_هندسة_إلكترونية"),
                ("🏭 المواد", "set_major_هندسة_مواد"),
                ("⚡ الطاقة", "set_major_هندسة_طاقة"),
            ],
            "computer": [
                ("💻 علوم الحاسوب", "set_major_علوم_حاسوب"),
                ("📱 تطوير البرمجيات", "set_major_تطوير_برمجيات"),
                ("🔒 أمن المعلومات", "set_major_امن_معلومات"),
                ("🤖 الذكاء الاصطناعي", "set_major_ذكاء_اصطناعي"),
                ("📊 علم البيانات", "set_major_علم_البيانات"),
            ],
            "business": [
                ("📈 إدارة الأعمال", "set_major_ادارة_اعمال"),
                ("💹 الاقتصاد", "set_major_اقتصاد"),
                ("📊 المحاسبة", "set_major_محاسبة"),
                ("🧮 التسويق", "set_major_تسويق"),
                ("🏦 التمويل", "set_major_تمويل"),
            ],
            "languages": [
                ("🌎 الإنجليزية", "set_major_لغة_انجليزية"),
                ("🇫🇷 الفرنسية", "set_major_لغة_فرنسية"),
                ("🇸🇦 العربية", "set_major_لغة_عربية"),
                ("📚 الترجمة", "set_major_ترجمة"),
                ("🇩🇪 الألمانية", "set_major_لغة_ألمانية"),
            ],
            "islamic": [
                ("📜 الفقه", "set_major_فقه"),
                ("💡 العقيدة", "set_major_عقيدة"),
                ("📖 التفسير", "set_major_تفسير"),
                ("🕌 الدراسات الإسلامية", "set_major_دراسات_اسلامية"),
                ("🌙 السيرة النبوية", "set_major_سيرة_نبوية"),
            ],
            "law": [
                ("📜 القانون الدولي", "set_major_قانون_دولي"),
                ("🏛️ القانون الوطني", "set_major_قانون_وطني"),
                ("🗳️ العلوم السياسية", "set_major_علوم_سياسية"),
                ("👮‍♂️ القانون الجنائي", "set_major_قانون_جنائي"),
                ("⚖️ القانون التجاري", "set_major_قانون_تجاري"),
            ],
            "science": [
                ("🧪 الفيزياء", "set_major_فيزياء"),
                ("🔬 الكيمياء", "set_major_كيمياء"),
                ("🔢 الرياضيات", "set_major_رياضيات"),
                ("🌿 الأحياء", "set_major_احياء"),
               ("🌎 الجيولوجيا", "set_major_جيولوجيا"),
            ],
            "arts": [
                ("🎭 الفنون الأدائية", "set_major_فنون_ادائية"),
                ("🖼️ الفنون البصرية", "set_major_فنون_بصرية"),
                ("📚 الأدب", "set_major_ادب"),
                ("🌏 التاريخ", "set_major_تاريخ"),
                ("🎵 الموسيقى", "set_major_موسيقى"),
            ],
            "education": [
                ("👩‍🏫 علم النفس التربوي", "set_major_علم_النفس_تربوي"),
                ("📘 المناهج وطرق التدريس", "set_major_مناهج_تدريس"),
                ("🧩 التربية الخاصة", "set_major_تربية_خاصة"),
                ("📊 إدارة التعليم", "set_major_ادارة_تعليم"),
                ("👶 الطفولة المبكرة", "set_major_طفولة_مبكرة"),
            ]
        }.get(category, [])
        ...
        for i in range(0, len(sub_specialties), 2):
            row = sub_specialties[i:i+2]
            if row:
                keyboard.row(
                    InlineKeyboardButton(row[0][0], callback_data=row[0][1]),
                    InlineKeyboardButton(row[1][0], callback_data=row[1][1]) if len(row) > 1 else None
        ...
        # إضافة أزرار إضافية
        keyboard.row(InlineKeyboardButton("❓ تخصص آخر", callback_data="set_major_custom"))
        keyboard.row(InlineKeyboardButton("⬅️ رجوع", callback_data="change_specialty"))
    
        # عناوين الفئات المصححة
        category_titles = {
            ...
            "law": "القانون والعلوم السياسية",
            "science": "العلوم الطبيعية",
            "arts": "الفنون والإنسانيات",
            "education": "التربية والتعليم"
        }
    
        bot.edit_message_text(
            f"🎯 *{category_titles.get(category, 'التخصصات')}*\n\n"
        ...
        if selected_major == "custom":
            ...
            bot.edit_message_text(
            "📝 يرجى إرسال اسم تخصصك (مثال: هندسة الطيران، علم البيانات)",
                chat_id=chat_id,
                message_id=message_id
            )
            return
        ...
        bot.edit_message_text(
            f"✅ تم تحديث تخصصك إلى: *{selected_major.replace('_', ' ')}*",
            chat_id=chat_id,
            message_id=message_id,
            reply_markup=keyboard,
            parse_mode="Markdown"
    ...
    # إنشاء لوحة مفاتيح متقدمة مع تنسيق أعمدة
        keyboard = InlineKeyboardMarkup(row_width=2)
    
    # تعريف التخصصات مع رموز تعبيرية
        specialties = [
            ("🩺 الطب والعلوم الصحية", "set_major_الطب_و_العلوم_الصحية"),
            ("🛠️ الهندسة والتكنولوجيا", "set_major_الهندسة_و_التكنولوجيا"),
            ("💊 الصيدلة", "set_major_الصيدلة"),
            ("💻 علوم الحاسوب", "set_major_علوم_الحاسوب"),
            ("📊 الإدارة والأعمال", "set_major_الإدارة_و_الأعمال"),
            ("🗣️ اللغات والترجمة", "set_major_اللغات_و_الترجمة"),
            ("🎓 التربية والتعليم", "set_major_التربية_و_التعليم"),
            ("⚖️ القانون", "set_major_القانون"),
            ("🎨 الفنون والإعلام", "set_major_الفنون_و_الإعلام"),
            ("🔬 العلوم الطبيعية", "set_major_العلوم_الطبيعية"),
            ("📿 الدراسات الإسلامية", "set_major_الدراسات_الإسلامية"),
            ("❓ تخصص آخر", "set_major_custom")
        ]
    
    # تنظيم الأزرار في صفوف متوازنة
        for i in range(0, len(specialties), 2):
            row_specialties = specialties[i:i+2]
            keyboard.row(
                InlineKeyboardButton(row_specialties[0][0], callback_data=row_specialties[0][1]),
                InlineKeyboardButton(row_specialties[1][0], callback_data=row_specialties[1][1]) if len(row_specialties) > 1 else None
            )
    
        # أزرار التنقل
        keyboard.row(InlineKeyboardButton("⬅️ رجوع", callback_data="go_account_settings"))

        # إرسال الرسالة بتنسيق احترافي
        try:
            bot.edit_message_text(
                "🎓 *اختر تخصصك الجديد*\n\n"
                "يمكنك اختيار تخصصك من القائمة التالية 👇\n"
                "سيتم تحديث إعدادات حسابك تلقائيًا.",
                chat_id=chat_id,
                message_id=message_id,
                reply_markup=keyboard,
                parse_mode="Markdown"
            ...
            print(f"خطأ في عرض تخصصات الإعدادات: {e}")

    elif data.startswith("set_major_"):
        selected_major = data.replace("set_major_", "")
     
        # معالجة التخصص المخصص
        if selected_major == "custom":
            user_states[uid] = "awaiting_custom_major"
            bot.edit_message_text(
                "📝 *أدخل تخصصك المخصص*\n\n"
                "من فضلك اكتب اسم تخصصك بدقة (مثال: هندسة الطيران، علم النفس السريري، إلخ).\n\n"
                "✅ سيتم حفظه كتخصصك الرئيسي.",
                chat_id=chat_id,
                message_id=message_id,
                parse_mode="Markdown"
            )
    ...
    # معالجة الصفحة الثانية للفئات
    elif data == "go_next":
        keyboard = InlineKeyboardMarkup(row_width=2)
        
        # الفئات التكميلية (الصفحة الثانية)
        buttons = [
            ("📿 العلوم الإسلامية", "category_العلوم_التربوية_الإسلامية_page2"),
            ("⚖️ القانون والسياسة", "category_القانون_والعلوم_السياسية_page2"),
            ("🧪 العلوم الطبيعية", "category_العلوم_الطبيعية_page2"),
            ("🎨 الفنون", "category_الفنون_page2"),
            ("👩‍🏫 التربية", "category_العلوم_التربوية_page2"),
        ]
        ...
        # أزرار التنقل
        keyboard.row(InlineKeyboardButton("❓ تخصص آخر", callback_data="major_custom"))
        keyboard.row(InlineKeyboardButton("⬅️ العودة", callback_data="go_prev"))
        keyboard.row(InlineKeyboardButton("🏠 القائمة الرئيسية", callback_data="go_back_home"))
        ...
        try:
            ...
            print(f"خطأ في عرض الصفحة الثانية: {e}")



    # معالجة العودة للصفحة الأولى
    elif data == "go_prev":
        keyboard = InlineKeyboardMarkup(row_width=2)
    ...
    # معالجة اختيار الفئة الرئيسية (معدلة بالكامل)
    elif data.startswith("category_"):
        # استخراج اسم الفئة والمصدر بشكل صحيح
        parts = data.split("_", 2)
        if len(parts) < 3:
            return  # تنسيق غير صحيح
        
        category = parts[1]
        source = parts[2]  # "page1" أو "page2"
        
        # قاموس شامل للتخصصات الفرعية
        CATEGORIES_DATA = {
            "العلوم_الصحية": [
                ("🧬 الطب البشري", "major_طب_بشري"),
                ("💊 الصيدلة", "major_صيدلة"),
                ("🏥 التمريض", "major_تمريض"),
                ("🔬 علوم المختبرات", "major_علوم_مختبرات"),
                ("🦷 طب الأسنان", "major_طب_أسنان"),
            ],
            "الهندسة": [
                ("⚙️ الهندسة الميكانيكية", "major_هندسة_ميكانيكية"),
                ("💡 الهندسة الكهربائية", "major_هندسة_كهربائية"),
                ("🏗️ الهندسة المدنية", "major_هندسة_مدنية"),
                ("🔧 هندسة البرمجيات", "major_هندسة_برمجيات"),
                ("📡 هندسة الإتصالات", "major_هندسة_اتصالات"),
            ],
            "علوم_الحاسوب": [
                ("💻 علوم الحاسوب", "major_علوم_حاسوب"),
                ("📱 تطوير البرمجيات", "major_تطوير_برمجيات"),
                ("🔒 أمن المعلومات", "major_امن_معلومات"),
                ("🤖 الذكاء الاصطناعي", "major_ذكاء_اصطناعي"),
                ("📊 علم البيانات", "major_علم_البيانات"),
            ],
            "العلوم_الادارية": [
                ("📈 إدارة الأعمال", "major_ادارة_اعمال"),
                ("💹 الاقتصاد", "major_اقتصاد"),
                ("📊 المحاسبة", "major_محاسبة"),
                ("🧮 التسويق", "major_تسويق"),
                ("🏦 التمويل", "major_تمويل"),
            ],
            "اللغات": [
                ("🌎 اللغة الإنجليزية", "major_لغة_انجليزية"),
                ("🇫🇷 اللغة الفرنسية", "major_لغة_فرنسية"),
                ("🇸🇦 اللغة العربية", "major_لغة_عربية"),
                ("📚 الترجمة", "major_ترجمة"),
                ("🇩🇪 اللغة الألمانية", "major_لغة_ألمانية"),
            ],
            "العلوم_التربوية_الإسلامية": [
                ("📜 الفقه", "major_فقه"),
                ("💡 العقيدة", "major_عقيدة"),
                ("📖 التفسير", "major_تفسير"),
                ("🕌 الدراسات الإسلامية", "major_دراسات_اسلامية"),
                ("🌙 السيرة النبوية", "major_سيرة_نبوية"),
            ],
            "القانون_والعلوم_السياسية": [
                ("📜 القانون الدولي", "major_قانون_دولي"),
                ("🏛️ القانون الوطني", "major_قانون_وطني"),
                ("🗳️ العلوم السياسية", "major_علوم_سياسية"),
                ("👮‍♂️ القانون الجنائي", "major_قانون_جنائي"),
                ("⚖️ القانون التجاري", "major_قانون_تجاري"),
            ],
            "العلوم_الطبيعية": [
                ("🧪 الفيزياء", "major_فيزياء"),
                ("🔬 الكيمياء", "major_كيمياء"),
                ("🔢 الرياضيات", "major_رياضيات"),
                ("🌿 الأحياء", "major_احياء"),
                ("🌎 الجيولوجيا", "major_جيولوجيا"),
            ],
            "الفنون": [
                ("🎭 الفنون الأدائية", "major_فنون_ادائية"),
                ("🖼️ الفنون البصرية", "major_فنون_بصرية"),
                ("📚 الأدب", "major_ادب"),
                ("🌏 التاريخ", "major_تاريخ"),
                ("🎵 الموسيقى", "major_موسيقى"),
            ],
            "العلوم_التربوية": [
                ("👩‍🏫 علم النفس التربوي", "major_علم_النفس_تربوي"),
                ("📘 المناهج وطرق التدريس", "major_مناهج_تدريس"),
                ("🧩 التربية الخاصة", "major_تربية_خاصة"),
                ("📊 إدارة التعليم", "major_ادارة_تعليم"),
                ("👶 الطفولة المبكرة", "major_طفولة_مبكرة"),
            ]
        }
        
        # الحصول على التخصصات الفرعية للفئة المحددة
        sub_buttons = CATEGORIES_DATA.get(category, [])
        
        # إعداد لوحة المفاتيح
        keyboard = InlineKeyboardMarkup(row_width=1)
        
        # إضافة الأزرار الفرعية
        for text, callback in sub_buttons:
        ...
        # تحديد زر الرجوع بناءً على المصدر
        back_callback = "go_generate" if source == "page1" else "go_next"
        keyboard.add(InlineKeyboardButton("⬅️ رجوع", callback_data=back_callback))
        
        # تحرير الرسالة مع تنسيق محسن
        try:
            ...
            try:
                ...
                print(f"خطأ جسيم في إرسال الرسالة: {e2}")


    elif data.startswith("major_"):
        major_key = data.replace("major_", "")
    
        if major_key == "custom":
            user_states[uid] = "awaiting_major"
            bot.edit_message_text(
            "✏️ من فضلك أرسل اسم تخصصك بدقة.",
                chat_id=chat_id,
                message_id=message_id,
                parse_mode="Markdown"
            )
            ...
            if major_key == "طب_بشري":
                user_states[uid] = "awaiting_human_medicine"
                keyboard = types.InlineKeyboardMarkup(row_width=2)
                keyboard.add(
                    types.InlineKeyboardButton("🧠 الوضع المتقدم", callback_data="advanced"),
                    types.InlineKeyboardButton("📄 الوضع المباشر", callback_data="simple")
                )
                ...
                user_states[uid] = {'message_id': message_id}
            ...
            if major_key != "طب_بشري":
                user_states[uid]['message_id'] = sent_msg.message_id

    elif data == "advanced":
        user_states[uid] = "awaiting_advanced_test_file"
        bot.edit_message_text(
    ...
    elif data.startswith("manual_anki"):
        # معالجة توليد يدوي
        bot.answer_callback_query(c.id)
        bot.edit_message_text(
...
def handle_user_major(msg):
    ...
    if state == "awaiting_major":
        bot.edit_message_text(uid, f"✅ تم تسجيل تخصصك: {major}\n"
                              "الآن أرسل ملف (PDF/DOCX/TXT) أو نصًا مباشرًا لتوليد اختبارك.", chat_id=chat_id, message_id=stored_message_id, parse_mode="Markdown" 
    )
...
def unified_handler(msg):
    ...
    try:
        ...
        # معالجة الصور (photo)
            ...
            bot.reply_to(msg, "🖼️ جاري استخراج النص من الصورة...")
            ...
            if ext == "pdf":
                ...
                bot.reply_to(msg, "⏳ جاري تحليل الصورة...")
                content, ocr_debug = extract_text_with_ocr_space(path, api_key=OCR_API_KEY, language="eng")
        ...
        if state == "awaiting_advanced_test_file":
            bot.send_message(msg.chat.id, "🤖 جاري معالجة الملف وإنشاء اختبار ذكي باستخدام الذكاء الاصطناعي...")
            ...
            if quiz_data:
                send_quiz_to_user(message.chat.id, quiz_data, loading_msg.message_id)
            else:
            ...
            user_states.pop(uid, None)
            if msg.content_type == "text":
                 # قائمة بنصوص انتظار رسمية وحيوية
                waiting_messages = [
                    "⏳ يتم تجهيز طلبك... يرجى الانتظار لحظة.",
                    "📊 يتم الأن معالجة البيانات... شكرًا لانتظارك.",
                    "🔍 تحليل وتجهيز ملف Anki الخاص بك...",
                    "⚡ نعمل على توليد البطاقات... لحظة من فضلك.",
                    "📈 جار تجهيز الملف... يرجى الانتظار قليلاً.",
                ]
                
                waiting_msg_text = random.choice(waiting_messages)
        
                # الرد الفوري برسالة انتظار مبدئية
                waiting_msg = bot.reply_to(msg, waiting_msg_text)
        
                 # قائمة بخطوات إنشاء الملف الفعلية بطريقة تفاعلية
                processing_steps = [
                    "🔍  تحليل النص.. ",
                    "⚙️  استخراج بطاقات ذكية... ",
                    "🎨  تنسيق البيانات بطريقة احترافية... ✨",
                    "🚀  توليد ملف Anki...",
                    "✅📦 تم تجهيز الملف النهائي للتحميل ",
                ]
        
                manual_input = msg.text
        
                # عرض خطوات المعالجة واحدة تلو الأخرى مع تأخير بسيط
                step_messages = []
                for step in processing_steps:
                    step_msg = bot.send_message(uid, step, parse_mode="Markdown")
                    step_messages.append(step_msg)
                    processing_time = time.sleep(random.randint(2, 5)) 
                # معالجة الملف الفعلي
                cards = parse_manual_anki_input(manual_input)
                if cards:
                    output_file = f"{uid}_manual_anki.apkg"
                    save_cards_to_apkg(cards, filename=output_file, deck_name="بطاقات المستخدم")
            
                    # حذف رسائل الانتظار والخطوات وإرسال الملف النهائي
                    bot.delete_message(uid, waiting_msg.message_id)
                    for step_msg in step_messages:
                        bot.delete_message(uid, step_msg.message_id)
                    bot.send_document(uid, open(output_file, 'rb'), caption="✅ ملف Anki الخاص بك جاهز!")
                else:
            ...
            if isinstance(quizzes, list) and len(quizzes) > 0:
                try:
                    ...
                    quiz_link = f"https://t.me/{BOT_USERNAME}?start=quiz_{quiz_code}"
