...
def can_generate(user_id: int) -> bool:
        ...
        for channel_id in allowed_channels:
            try:
                ...
                if member.status in ['member', 'administrator', 'creator']:
                    break  # ✅ مستخدم عضو في إحدى القنوات
            except Exception as e:
                logging.warning(f"⚠️ فشل التحقق من القناة {channel_id} للمستخدم {user_id}: {e}")
        else:
            return False  # ❌ المستخدم ليس عضواً في أي قناة
    except Exception as e:
        logging.error(f"🚫 خطأ في قراءة القنوات المسموح بها: {e}")
        return False

    # ✅ التحقق من عدد المحاولات مع إعادة التعيين الشهري
    ...
    if not row:
        # 🆕 مستخدم جديد: أنشئ له سجلًا
        cursor.execute(
            "INSERT INTO users (user_id, quiz_count, last_reset) VALUES (?, ?, ?)",
    ...
    if not last_reset_date or last_reset_date.month != today.month or last_reset_date.year != today.year:
        cursor.execute(
            "UPDATE users SET quiz_count = 0, last_reset = ? WHERE user_id = ?",
            (today.isoformat(), user_id)
        )
        conn.commit()
        quiz_count = 0

    return quiz_count < MAX_FREE_ATTEMPTS
...
class QuizManager:
    ...
    def start_quiz(self, chat_id, quiz_code, bot, message_id=None):
        conn = sqlite3.connect("quiz_users.db")
        c = conn.cursor()
        ...
        if not result:
            if message_id:
                bot.edit_message_text(chat_id=chat_id, message_id=message_id, text="❌ الاختبار غير موجود أو منتهي الصلاحية")
            else:
                bot.send_message(chat_id, "❌ الاختبار غير موجود أو منتهي الصلاحية")
            return False
        ...
        if message_id:
            ...
            bot.send_message(chat_id, f"تم توليد {len(formatted_quizzes)} سؤالاً، استعد للإختبار")
        self.send_question(chat_id, bot)
        return True
    ...
    def send_question(self, chat_id, bot):
        ...
        try:
            ...
            self.handle_quiz_end(chat_id, bot, error=True)
    ...
    def handle_answer(self, poll_answer, bot):
        ...
        if state['current_index'] < len(state['quizzes']):
            ...
            self.handle_quiz_end(chat_id, bot)
    
    def handle_quiz_end(self, chat_id, bot, error=False):
        state = self.active_quizzes.pop(chat_id, None)
        if not state:
            return
            
        if error:
            bot.send_message(chat_id, "⚠️ حدث خطأ في الاختبار. يرجى المحاولة لاحقًا")
            return
...
def handle_poll_answer(poll_answer):
    quiz_manager.handle_answer(poll_answer, bot)
...
def send_quizzes(chat_id, quizzes, message_id=None):
    try:
        # إرسال رسالة التحميل
        msg = bot.send_message(chat_id, "⏳ جاري تحضير الاختبار...")
        ...
        # بدء الاختبار
        if quiz_manager.start_quiz(chat_id, quiz_code, bot):
            bot.delete_message(chat_id, msg.message_id)
        else:
            bot.edit_message_text("❌ فشل بدء الاختبار", chat_id, msg.message_id)
...
# معالجة القائمة الرئيسية
    if data == "go_generate":
        ...
        buttons = [
            ("🩺 الطب و العلوم الصحية", "category_العلوم_الصحية"),
            ("🛠️ الهندسة", "category_الهندسة"),
            ("💻 علوم الحاسوب", "category_علوم_الحاسوب"),
            ("📊 العلوم الإدارية والإقتصادية", "category_العلوم_الادارية"),
            ("🗣️ اللغات", "category_اللغات"),
        ...
        keyboard.add(InlineKeyboardButton("⬅️ التالي", callback_data="go_next"))


        category1_msg = bot.edit_message_text(
            "🎯 *TestGenie* يساعدك على توليد اختبارات ذكية من ملفاتك الدراسية أو النصوص.\n\n\n"
            "اختر تخصصك الأساسي للبدء 👇",
            chat_id=chat_id,
            message_id=message_id,
            reply_markup=keyboard,
            parse_mode="Markdown"
        ...
        choice_markup.row(
                types.InlineKeyboardButton("📝 توليد يدوي", callback_data=f"manual_anki_{uid}"),
            types.InlineKeyboardButton("🤖 توليد آلي", callback_data=f"ai_anki_{uid}")
        )
    ...
    # إعادة إرسال الواجهة الرئيسية بعد ثوانٍ
    ...
    elif data.startswith("go_next"):
        keyboard = InlineKeyboardMarkup()
        buttons = [
            ("📿 العلوم التربوية الإسلامية", "category_العلوم_التربوية_الإسلامية"),
            ("⚖️ القانون والعلوم السياسية", "category_الهندسة"),
            ("🧬 العلوم الطبيعية", "category_العلوم_الطبيعية"),
            ("🎨 الفنون والإنسانيات", "category_الفنون"),
            ("📚 العلوم التربوية", "category_العلوم_التربوية"),
        ...
        keyboard.add(InlineKeyboardButton("➡️ السابق", callback_data="go_prev"))


        category2_msg = bot.edit_message_text(
            "🎯 *TestGenie* يساعدك على توليد اختبارات ذكية من ملفاتك الدراسية أو النصوص.\n\n\n"
            "اختر تخصصك الأساسي للبدء 👇",
            chat_id=chat_id,
            message_id=message_id,
...
# معالجة اختيار الفئة الرئيسية
        ...
        elif category == "العلوم التربوية الإسلامية":
            sub_buttons = [
                ("📜 الفقه", "major_فقه"),
                ("💡 العقيدة", "major_عقيدة"),
                ("📖 التفسير", "major_تفسير"),
        ...
        elif category == "القانون والعلوم السياسية":
            sub_buttons = [
        ...
        elif category == "الفنون والإنسانيات":
            sub_buttons = [
        ...
        for text, data_btn in sub_buttons:
            keyboard.add(InlineKeyboardButton(text, callback_data=data_btn))
        if source == "source1":
            keyboard.add(InlineKeyboardButton("⬅️ رجوع", callback_data="go_generate"))
        elif source == "source2":
            keyboard.add(InlineKeyboardButton("⬅️ رجوع", callback_data="go_next"))
        bot.edit_message_text(
            f"👈 اختر تخصصًا دقيقًا ضمن {category}:",
            chat_id=chat_id,
            message_id=message_id,
            reply_markup=keyboard,
            parse_mode="Markdown"
        ...
        if major_key == "custom":
            ...
            bot.edit_message_text("✏️ من فضلك أرسل اسم تخصصك بدقة.",
                chat_id=chat_id,
                message_id=message_id,
                parse_mode="Markdown"
        )
            ...
            conn.commit()
            sent_msg = bot.edit_message_text(f"✅ تم تحديد تخصصك: {major_key}\n"
                             "الآن أرسل ملف (PDF/DOCX/TXT) أو نصًا مباشرًا لتوليد اختبارك.",chat_id=chat_id, message_id=message_id, parse_mode="Markdown"
                             )
