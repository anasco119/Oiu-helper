...
def worker():
    while True:
        msg, sent_msg = request_queue.get()
        try:
            process_message(msg, sent_msg.message_id, sent_msg.chat_id)
        except Exception as e:
...
def process_message(msg, message_id=None, chat_id=None):
    ...
    with state_lock:
            state = user_states.get(uid)
    logging.info("Current state for uid %s: %s", uid, state)
    ...
    try:
        ...
        if state == "awaiting_anki_file_ai":
            ...
            loading_msg = bot.edit_message_text("🔄 جاري معالجة الملف...", chat_id=chat_id, message_id=message_id)
        ...
        # ============================
            ...
            if msg.content_type == "text":
                ...
                waiting_msg = bot.edit_message_text(chat_id=chat_id, message_id=message_id, text=waiting_msg_text)
