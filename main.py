...
def send_comment(action: str, username: str, user_id: int, message: str):
    """
    يرسل إشعار للإدمن بخصوص عملية معينة.
    action: نوع العملية (مثلاً: "توليد أنكي آلي")
    """
    ...
    bot3.send_message(
        ADMIN_ID,
        f"👥💬 لديك {action} جديد من: {username} | UID: {user_id}\n\n{action}: {message}"
    )
...
def generate_Medical_quizzes(content: str, major: str, user_id: int, quiz_level: str, num_quizzes: int = 10):
    ...
    prompt = (
        ...
        "2. 90% multiple-choice (basic sciences), 10% problem-solving (clinical case also multiple choices).\n" 
    ...
    def start_quiz(self, chat_id, quiz_code, bot2, message_id=None):
        try:
            ...
            if user_id is not None and chat_id == user_id:
                ...
                # قائمة بجميع الملاحظات والنصائح المتاحة
                tips = [
                    "💡 يمكنك التحكم في مستوى الاختبار في إعدادات البوت.",
                    "🔹 قريباً، استعد لتجربة تعليمية فريدة مع الأستاذ المحاكي.",
                    "تخيل أنك في قاعة دراسية مع أستاذ خاص بك. هذا ما سيقدمه لك البوت قريباً.",
                    "📌 TestGenie يقوم بتحويل ملفاتك الدراسية إلى وسائل دراسية فعّالة.",
                    "💡 تذكر أن الممارسة المستمرة هي مفتاح الإتقان. حاول إجراء اختبارات دورية."
                ]



                # اختيار ملاحظة واحدة عشوائياً من القائمة
                random_tip = random.choice(tips)

                # الرسالة النهائية بالتنسيق المضمون
                msg = (
                    f"<b>📝 ملخص الاختبار 📝</b>\n\n"
                    f"✅ <b>العدد الإجمالي للأسئلة:</b> {len(formatted_quizzes)}\n\n"
                    f"حان الآن وقت اختبار معلوماتك. استعد جيدًا، وخذ وقتك في الإجابة.\n\n"
                    f"<b>بالتوفيق!</b>\n"
                    f"──────────────────\n"
                    f"{random_tip}"
                )
...
def unified_start_handler(message):
    ...
    if len(args) > 1:
        ...
        if param == "anki_sample":
            ...
            bot.send_message(text="📝 دعنا نبدأ بإنشاء *ملف بطاقاتك الأول*!\n"
...
def handle_rating(call):
    ...
    if rating == "ignore":
        ...
        bot.edit_message_text(chat_id=chat_id, text="✅ تم تجاهل التقييم", message_id=message_id)
        time.sleep(1)
        try:
        ...
        bot.edit_message_text(chat_id=chat_id, text=text_low, message_id=message_id)
        user_states[uid] = "awaiting_comment"
        
    elif rating == "3":
        bot.edit_message_text(chat_id=chat_id, text=text_mid, message_id=message_id)
        user_states[uid] = "awaiting_comment"
    ...
    send_comment("تقييم", username, uid, user_rating)

    bot.answer_callback_query(call.id, "✅ تم تسجيل تقييمك، شكراً لك!")


# استقبال رسالة المستخدم بعد التقييم (للتعليقات)
@bot.message_handler(func=lambda message: user_states.get(message.from_user.id) == "awaiting_comment")
def handle_user_comment(message):
    uid = message.from_user.id
...
def handle_main_menu(c):
    ...
    # ... بقية المعالجات الأخرى ...
        ...
        if data == "go_account_settings":
            ...
            settings_keyboard.add(
                InlineKeyboardButton("📉 مستوى الإختبارات", callback_data="test_level"),
            )
            settings_keyboard.add(
...
def process_message(msg, message_id=None, chat_id=None):
    ...
    try:
        if msg.content_type == "text":
            content = msg.text or ""
            coverage = "كاملة ✅"
            if len(content.strip()) < 20: # يمكنك تغيير هذا الرقم حسب الحاجة
                bot.edit_message_text(
                    "⚠️ النص قصير جداً. يرجى إرسال نص أطول أو ملف.",
                    chat_id=chat_id,
                    message_id=message_id
                )
