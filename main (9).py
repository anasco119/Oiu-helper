...
class QuizManager:
    ...
    def handle_quiz_end(self, chat_id, bot, error=False):
        ...
        keyboard.row(
            ...
            types.InlineKeyboardButton(
                "📤 مشاركة الاختبار",
                url=f"https://t.me/Oiuhelper_bot?start=quiz_{state['quiz_code']}"
            )
        )
        keyboard.row(
            types.InlineKeyboardButton(
...
def unified_start_handler(message):
    ...
    if len(args) > 1:
        ...
        if param.startswith("quiz_"):
            ...
            if quiz_manager.start_quiz(message.chat.id, quiz_code, bot):
                bot.send_message(message.chat.id, "🧠 هذا اختبار تمت مشاركته معك. استعد!")
            else:
                bot.send_message(message.chat.id, "❌ لم يتم العثور على هذا الاختبار أو انتهت صلاحيته.")
            return
